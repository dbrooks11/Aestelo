...
class SpotService(SQLAlchemyAsyncRepositoryService[Spot]):
    ...
    async def get_spots_me_pagination(
        self,
        user_id: str,
        page: int,
        page_size: int,
        ...
        visit_subqeury = (
            select(1).where(Visit.spot_id == Spot.id, Visit.user_id == user_id).exists()
        )
        rated_subquery = (
            select(1)
            .where(Rating.spot_id == Spot.id, Rating.user_id == user_id)
            .exists()
        )
        saved_subquery = (
            select(1)
            .where(
                CollectionItem.spot_id == Spot.id, CollectionItem.saved_by == user_id
            )
            .exists()
        )
        ...
        stmt = (
            select(Spot)
            .where(Spot.user_id == user_id, Spot.status == UploadStatusEnum.SUCCESS)
            .options(
...
async def provide_spot_service(
    db_session: NamedDependency[AsyncSession],
) -> AsyncGenerator[SpotService]:
...
async def provide_spot_media_service(
    db_session: NamedDependency[AsyncSession],
) -> AsyncGenerator[SpotMediaService]:
