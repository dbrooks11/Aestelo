...
class UserProfileService(SQLAlchemyAsyncRepositoryService[UserProfile]):
    ...
    async def get_profile_me(self, user_id: str) -> UserProfile:
        return await self.get(
            item_id=user_id,
            error_messages=ErrorMessages({"not_found": "Failed to fetch profile"}),
            load=[joinedload(UserProfile.auth)],
        )

    async def update_profile(
        self, user_id: str, data: UserProfileEditSchema
    ) -> UserProfile:
        ...
        return await self.update(
            data=temp_data,
            item_id=user_id,
            error_messages=ErrorMessages({"other": "Failed to update profile"}),
        )
...
) -> AsyncGenerator[UserProfileService]:
