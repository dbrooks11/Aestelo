...
class VisitService(SQLAlchemyAsyncRepositoryService[Visit]):
    ...
    async def get_visits_me_pagination(
        self,
        user_id: str,
        page: int,
        page_size: int,
        ...
        return OffsetPagination(
            items=items, limit=page_size, offset=offset, total=total
        )
...
async def provide_visit_service(
    db_session: NamedDependency[AsyncSession],
) -> AsyncGenerator[VisitService]:
...
async def provide_visit_media_service(
    db_session: NamedDependency[AsyncSession],
) -> AsyncGenerator[VisitMediaService]:
