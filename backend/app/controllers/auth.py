...
class AuthController(Controller):
    ...
    async def signup(
        ...
        if exist_username:
            raise InternalServerException(
                detail="Failed to create an account. Please try again."
            )
        ...
        await auth_service.create_account(
            data=AuthServiceSignupSchema.model_validate(data_dict)
        )
    ...
    async def login(
        ...
        account = await db_session.execute(
            select(AuthUser.id, AuthUser.password_hash).where(
                (AuthUser.email == data.email) | (AuthUser.username == data.username)
            )
        )
        row = account.first()
        if not row:
        ...
        update_fields: dict[str, str | now] = {"last_signed_in": func.now()}
