...
class Spot(base.BigIntAuditBase):
    __tablename__ = "spot"
    __table_args__ = (
        Index("ix_spot_color_palette", "color_palette", postgresql_using="gin"),
        Index("ix_spot_hashtags", "hashtags", postgresql_using="gin"),
    )

    visited: Mapped[bool | None] = query_expression()
    rated: Mapped[bool | None] = query_expression()
    saved: Mapped[bool | None] = query_expression()
    ...
    color_palette: Mapped[ARRAY | None] = mapped_column(ARRAY(Text))

    description: Mapped[str | None] = mapped_column(
        String(validate.MAX_POST_DESCRIPTION_LENGTH)
    )
    ...
    status: Mapped[Enum] = mapped_column(
        Enum(UploadStatusEnum), default=UploadStatusEnum.PROCESSING
    )
    ...
    collection_item: Mapped["CollectionItem"] = relationship(
        back_populates="spot", lazy="selectin"
    )
...
class SpotMedia(base.BigIntBase):
    ...
    spot_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("spot.id", ondelete="CASCADE"), index=True
    )
    uploaded_by: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("user_profile.id"), index=True
    )
