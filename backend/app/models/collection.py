...
class Collection(base.BigIntAuditBase):
    ...
    collection_item: Mapped[list["CollectionItem"]] = relationship(
...
class CollectionItem(base.BigIntBase):
    ...
    __table_args__ = (
        UniqueConstraint(
            "collection_id", "spot_id", name="uq_collection_item_collection_spot"
        ),
        UniqueConstraint(
            "collection_id", "visit_id", name="uq_collection_item_collection_visit"
        ),
    )
    ...
    spot_id: Mapped[int | None] = mapped_column(
        BigInteger, ForeignKey("spot.id", ondelete="CASCADE"), nullable=True
    )
    visit_id: Mapped[int | None] = mapped_column(
        BigInteger, ForeignKey("visit.id", ondelete="CASCADE"), nullable=True
    )
