...
class ObjectStorage:
    ...
    async def verify_file_type(self, mimetype: str | None, is_post: bool):
        ...
        extension: str | None = (
            ALLOWED_MIME_TYPES.get(mimetype, None)
            if not is_post
            else ALLOWED_POST_MIME_TYPES.get(mimetype, None)
        )
        if not extension:
    ...
    async def generate_file_name(
        self, id: str | int, extension: str, folder: str = "quarantine"
    ) -> str:
    ...
    async def generate_presigned_put_url(
        self, mimetype: str, obj_key: str, expires_in: int = 600
    ) -> dict[str, str]:
        ...
        async with self.session.client(
            ...
            response = await s3.generate_presigned_url(
                "put_object",
                Params={"Bucket": self.bucket, "Key": obj_key, "ContentType": mimetype},
    ...
    async def download_file_from_s3(self, key: str, local_path: str):
        ...
        async with self.session.client(
            ...
            await s3.download_file(Bucket=self.bucket, Key=key, Filename=local_path)
    ...
    async def upload_file_s3(self, object_key: str, file: bytes, mimetype: str) -> bool:
        ...
        async with self.session.client(
            ...
            await s3.put_object(
                Bucket=self.bucket, Key=object_key, Body=file, ContentType=mimetype
            )
            return True
