...
class GPSHandling:
    ...
    async def parse_gps_data_pyvips(
    ...
    async def get_lat_long(
        ...
        parsed_gps_data: (
            dict[str, str | list[str]] | None
        ) = await self.parse_gps_data_pyvips(exif=exif)
        ...
        async def dms_to_decimal(
            ref: str, decimal: str, minute: str, second: str
        ) -> float | None:
            decimal_result: float = (
                float(decimal) + (float(minute) / 60) + (float(second) / 3600)
            )
        ...
        return {
            self.LOCATION_KEY_LAT: await dms_to_decimal(
                ref=lat_ref, decimal=lat_d, minute=lat_m, second=lat_s
            ),
            self.LOCATION_KEY_LON: await dms_to_decimal(
                ref=long_ref, decimal=long_d, minute=long_m, second=long_s
            ),
        }
    ...
    async def average_location(self, coords: list[dict[str, float]]) -> dict | None:
        ...
        filtered = [
            ...
            for c in coords
            if geodesic(
                (c[self.LOCATION_KEY_LAT], c[self.LOCATION_KEY_LON]), (avg_lat, avg_lon)
            ).meters
            <= threshold_meters
        ]
