...
async def process_post_media(post_type: Literal['spot','visit'], post_id: int, sort_order: int, obj_key: str, user_id: str):

    async with media_semaphore:
        is_uploaded: bool = False 
        new_obj_key: str = ''
        try:
            processed_media = await process_media_image(storage=storage_private, obj_key=obj_key, need_exif=True, need_colors=True)

            compressed_file: Any | None = processed_media.get('compressed_file', None)
            original_mimetype: str | None = processed_media.get('original_mimetype', None)
...
@monitored_job(1.0)
async def process_post_pipeline(ctx: Context, *, tasks: list[dict[str, str | int]], post_type: Literal['spot','visit'], post_id: int, user_id: str):
        queued_task: list = []
        ...
        try:
            for task in tasks:
                ...
                if sort_order and key:
                    queued_task.append(process_post_media(post_type=post_type, post_id=post_id,
                                                        sort_order=sort_order, obj_key=key,
                                                        user_id=user_id))
            ...
            if not avg_coords:
                raise ValidationException(detail='Distance between media varies greatly. Please make sure that all media was taken within proximity of each other')
            ...
            async with get_db_session() as session:
                if post_type == 'spot':
                    post_service = SpotService(session=session)  
                    await post_service.update_spot(data={'status': UploadStatusEnum.FAILED}, spot_id=post_id)  
                else: 
                    post_service = VisitService(session=session)
            raise
