...
@monitored_job()
async def process_profile_media(
    ctx: Context, *, user_id: str, field: str, obj_key: str, prev_key: str
):
    uploaded: bool = False
    new_obj_key: str = ""

    try:
        processed_file_data = await process_media_image(
            storage=storage_private, obj_key=obj_key, need_exif=False, need_colors=False
        )
        ...
        valid_extension = await storage.verify_file_type(
            mimetype=original_mimetype, is_post=False
        )
        new_obj_key = await storage.generate_file_name(
            folder=field, id=user_id, extension=valid_extension
        )

        uploaded = await storage.upload_file_s3(
            object_key=new_obj_key, file=processed_file, mimetype=mimetype
        )
        ...
        async with get_db_session() as session:
            ...
            updated = await profile_service.update(
                data={field: new_obj_key}, item_id=user_id
            )
